...
function doPost(e) {
  try {
    ...
    if (action === 'syncFullDatabase') {
      ...
      if (pacientes) replaceSheetData(SHEETS.PACIENTES, pacientes.map(p => [
        p.id, p.cedula, p.nombre, p.genero, p.correo, p.telefono,
        p.contactada, p.fecha, p.promocion, p.procedimiento, p.direccion
      ]));

      if (pagos) replaceSheetData(SHEETS.PAGOS, pagos.map(p => [
      ...
      if (usuarios) replaceSheetData(SHEETS.USUARIOS, usuarios.map(u => [
        u.usuarioId, u.nombre, u.email, u.passwordHash, u.rol, u.estatus, u.fechaCreacion
      ]));

      if (actividades) replaceSheetData(SHEETS.ACTIVIDADES, actividades.map(c => [
        c.actividadId, c.pacienteId, c.tipoActividad, c.descripcion,
        c.fechaProgramada, c.hora, c.estado, c.alarma ? 'Sí' : 'No', c.responsableId
      ]));

      if (financiamientos) replaceSheetData(SHEETS.FINANCIAMIENTO, financiamientos.map(f => [
...
function updateRowById(sheetName, idColumnIndex, targetId, newRowArray) {
  ...
  for (let i = 1; i < values.length; i++) {
    if (String(values[i][idColumnIndex]) === String(targetId)) {
      sheet.getRange(i + 1, 1, 1, newRowArray.length).setValues([newRowArray]);
      return;
    }
  }
