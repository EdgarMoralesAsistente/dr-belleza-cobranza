            ...
            <div className="relative">
              ...
              >
                <div className="w-8 h-8 rounded-lg bg-teal-100 text-teal-800 font-bold text-xs flex items-center justify-center border border-teal-200 overflow-hidden">
                  {currentUser?.fotoUrl ? (
                    <img src={currentUser.fotoUrl} alt={currentUser.nombre || 'Usuario'} className="w-full h-full object-cover" />
                  ) : (
                    (currentUser?.nombre || 'US').substring(0, 2).toUpperCase()
                  )}
                </div>
                <div className="hidden lg:block">
                  ...
                  <span className={`inline-block text-[10px] font-medium border px-1.5 py-0.2 rounded-md ${getRoleBadgeColor(currentUser?.rol || 'Asistente')}`}>
                ...
                <div className="absolute right-0 mt-2 w-64 bg-white rounded-2xl shadow-xl border border-slate-200 py-2 z-50 animate-in fade-in duration-150">
                  <div className="px-3 py-2.5 border-b border-slate-100 flex items-center space-x-2.5">
                    ...
                    <div className="min-w-0 flex-1">
                      <p className="text-xs font-bold text-slate-900 truncate">{currentUser?.nombre || 'Usuario'}</p>
                      <p className="text-[10px] text-slate-500 truncate">{currentUser?.email || ''}</p>
                      <span className={`inline-block mt-0.5 text-[9px] font-bold border px-1.5 py-0.2 rounded-md ${getRoleBadgeColor(currentUser?.rol || 'Asistente')}`}>
                        {currentUser?.rol || 'Asistente'}
                      </span>
                    </div>
                  </div>
